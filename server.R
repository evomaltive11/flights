server <- function(input, output, session) {
  ...
  showNotification("Created by Nicholas Andrian", duration = NULL, type = "message")
...
# Total Flights (server) ------------------------------------------
  output$total_flights <- renderValueBox({
    ...
    base_flights() %>%
      ...
      as.integer() %>%
      prettyNum(big.mark = ",") %>%
      valueBox(icon = icon("chart-bar"), color = "olive",subtitle = "Number of Flights")
  })
...
# Avg per Day (server) --------------------------------------------
  output$per_day <- renderValueBox({
    ...
    base_flights() %>%
      ...
      round() %>%
      prettyNum(big.mark = ",") %>%
      valueBox(icon = icon("balance-scale"), color = "orange",subtitle = "Average Flights per day") 
  })
...
# Percent delayed (server) ----------------------------------------
  output$percent_delayed <- renderValueBox({
    base_flights() %>%
      ...
      round() %>%
      paste0("%") %>%
      valueBox(icon = icon("percent"), color = "red", subtitle = "Flights delayed")
  })
